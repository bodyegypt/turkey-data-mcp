...
function checkRateLimit() {
    ...
    if (requestTimestamps.length >= MAX_REQUESTS_PER_WINDOW) {
        ...
        throw new Error(`Client-side rate limit reached (${MAX_REQUESTS_PER_WINDOW} requests/min to turkpidya.com). ` +
...
async function apiGet(path, params = {}) {
    ...
    try {
        ...
        throw new Error(`Could not reach the turkpidya.com data API (${reason}). ` +
            `Check network connectivity and retry; the API itself is normally up 24/7.`);
    }
    if (response.status === 429) {
        throw new Error("The turkpidya.com API returned HTTP 429 (rate limited: 60 requests/min per IP). " +
    ...
    try {
        ...
        throw new Error(`The API returned a non-JSON response (HTTP ${response.status}): ${text.slice(0, 200)}`);
    }
    if (!response.ok) {
        const apiError = body;
...
server.registerTool("get_gold_prices", {
    title: "Turkey Gold & Silver Prices",
    description: "Get current gold and silver prices in Turkey, quoted in Turkish lira (TRY). " +
    ...
    inputSchema: {
        category: z
            ...
            .optional()
            .describe("Filter by category: 'gram' = gold per gram (14k–24k), 'coin' = Turkish gold coins " +
...
server.registerTool("get_exchange_rates", {
    ...
    description: "Get official Turkish lira (TRY) exchange rates from TCMB, the Central Bank of the " +
    ...
    inputSchema: {
        currency: z
            ...
            .optional()
            .describe("Optional 3-letter ISO 4217 currency code (e.g. 'USD', 'EUR', 'SAR'). " +
...
server.registerTool("get_fuel_prices", {
    title: "Turkey Fuel Pump Prices",
    description: "Get current pump fuel prices in Turkey in TRY per litre, aggregated from official EPDK " +
    ...
    inputSchema: {
        city: z
            ...
            .optional()
            .describe("Optional city filter. One of: istanbul, ankara, izmir, antalya. " +
...
server.registerTool("get_prayer_times", {
    title: "Prayer Times for Turkish Cities",
    description: "Get Islamic prayer times (namaz vakitleri) for a Turkish city using the official " +
    ...
    inputSchema: {
        city: z
            .string()
            .min(2)
            .describe("City slug, lowercase ASCII, e.g. 'istanbul', 'izmir', 'diyarbakir', 'sanliurfa'. " +
        ...
        date: z
            ...
            .optional()
            .describe("Optional date in YYYY-MM-DD format. Defaults to today (Europe/Istanbul). " +
            "Only dates near today are available."),
    },
}, async ({ city, date }) => {
    try {
        return jsonResult(await apiGet("/prayer-times", { city: city.toLowerCase().trim(), date }));
    }
    catch (err) {
        return errorResult(err);
    }
});
server.registerTool("list_prayer_cities", {
    title: "List Supported Prayer-Time Cities",
    description: "List the Turkish city slugs supported by get_prayer_times (20 major cities, e.g. " +
...
server.registerTool("get_earthquakes", {
    ...
    description: "Get recent earthquakes in and around Turkey from the Kandilli Observatory (KOERI) " +
    ...
    inputSchema: {
        ...
        city: z
            .string()
            .optional()
            .describe("Case-insensitive substring filter on the location name, e.g. 'izmir', " +
    ...
    try {
        return jsonResult(await apiGet("/earthquakes", { hours, min_magnitude, city, limit }));
    }
    catch (err) {
        return errorResult(err);
    }
});
server.registerTool("get_latest_earthquake", {
    title: "Latest Earthquake in Turkey",
    description: "Get the single most recent earthquake recorded in or around Turkey by the Kandilli " +
